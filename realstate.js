// Mock Data
const categories = [
  { id: 1, name: "Real Estate", icon: "fas fa-home", count: 245, color: "#3b82f6" },
  { id: 2, name: "Automotive", icon: "fas fa-car", count: 189, color: "#ef4444" },
  { id: 3, name: "Electronics", icon: "fas fa-laptop", count: 334, color: "#8b5cf6" },
  { id: 4, name: "Fashion", icon: "fas fa-tshirt", count: 167, color: "#ec4899" },
  { id: 5, name: "Services", icon: "fas fa-tools", count: 223, color: "#10b981" },
  { id: 6, name: "Food & Beverage", icon: "fas fa-utensils", count: 145, color: "#f59e0b" },
  { id: 7, name: "Health & Beauty", icon: "fas fa-heart", count: 178, color: "#06b6d4" },
  { id: 8, name: "Sports & Recreation", icon: "fas fa-football-ball", count: 156, color: "#84cc16" },
  { id: 9, name: "Education", icon: "fas fa-graduation-cap", count: 89, color: "#6366f1" },
  { id: 10, name: "Travel & Tourism", icon: "fas fa-plane", count: 134, color: "#14b8a6" },
  { id: 11, name: "Agriculture", icon: "fas fa-seedling", count: 198, color: "#22c55e" },
  { id: 12, name: "Construction", icon: "fas fa-hard-hat", count: 167, color: "#f97316" },
]
...
const listings = [
  ...
  },
  {
    ...
    description: "Brand new MacBook Pro with M2 chip, 16GB RAM, 512GB SSD. Ideal for professionals and students.",
  ...
  },
  {
    ...
    description: "Handwoven traditional dresses with intricate patterns. Perfect for cultural events and celebrations.",
    category: "Fashion",
    location: "Gondar",
    price: 3500,
    image: "/placeholder.svg?height=220&width=380",
  ...
  },
  {
    ...
    description: "Fully furnished office space in prime location with high-speed internet, meeting rooms, and parking.",
    category: "Real Estate",
    location: "Mekelle",
    price: 12000,
    image: "/placeholder.svg?height=220&width=380",
...
function renderCategories() {
  ...
  categoriesGrid.innerHTML = categories
    .map(
      (category, index) => `
        <div class="category-card fade-in" onclick="filterByCategory('${category.name}')" style="animation-delay: ${index * 0.1}s">
...
function renderListings() {
  ...
  listingsGrid.innerHTML = filteredListings
    .map(
      (listing, index) => `
        <div class="listing-card fade-in" onclick="showListingDetails(${listing.id})" style="animation-delay: ${index * 0.1}s">
            ...
            <div class="listing-content">
                ...
                <div class="listing-stats">
                    ...
                    <span class="listing-date">${formatDate(listing.date)}</span>
                </div>
                <div class="broker-info">
                    <img src="${listing.broker.avatar}" alt="${listing.broker.name}" class="broker-avatar">
                    <div class="broker-details">
                        <h4>${listing.broker.name}</h4>
                        <div class="broker-rating">
                            ${"★".repeat(Math.floor(listing.broker.rating))} ${listing.broker.rating}
                        </div>
                        <div class="broker-experience">${listing.broker.experience} experience</div>
                    </div>
                    ${listing.broker.verified ? '<span class="verified-badge"><i class="fas fa-check"></i> Verified</span>' : ""}
...
function renderTestimonials() {
  ...
  sliderDots.innerHTML = testimonials
    .map(
      (_, index) => `
        <button class="slider-dot ${index === 0 ? "active" : ""}" onclick="goToTestimonial(${index})" aria-label="Go to testimonial ${index + 1}"></button>
    `,
    )
    .join("")
...
function setupSearchSuggestions() {
  ...
  searchInput.addEventListener("input", function () {
    ...
    const matches = searchSuggestions.filter((suggestion) => suggestion.toLowerCase().includes(query)).slice(0, 5)
...
function performAdvancedSearch() {
  ...
  filteredListings = listings.filter((listing) => {
    ...
    if (priceRange) {
      const [min, max] = priceRange.split("-").map((p) => Number.parseInt(p.replace("+", "")))
      if (max) {
  ...
  showNotification(`Found ${filteredListings.length} listings matching your search`, "info")
  ...
  const detailsHTML = `
        <div style="padding: 2rem;">
            <div class="listing-image-gallery">
                <img src="${listing.image}" alt="${listing.title}" style="width: 100%; height: 400px; object-fit: cover; border-radius: 12px; margin-bottom: 2rem;">
            </div>
            ...
            <div style="display: grid; grid-template-columns: 2fr 1fr; gap: 3rem;">
                <div class="listing-main-info">
                    <div class="listing-badges" style="display: flex; gap: 1rem; margin-bottom: 1rem;">
                        ...
                        ${listing.featured ? '<span style="background: var(--warning-color); color: white; padding: 0.5rem 1rem; border-radius: 20px; font-size: 0.9rem; font-weight: 600;"><i class="fas fa-star"></i> Featured</span>' : ""}
                    </div>
                    
                    <h2 style="font-size: 2.2rem; margin-bottom: 1rem; color: var(--text-primary); font-weight: 700;">${listing.title}</h2>
                    ...
                    <div class="listing-price-location" style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 2rem; padding: 1.5rem; background: var(--bg-tertiary); border-radius: 12px;">
                        <div>
                            <span style="font-size: 2rem; font-weight: 800; color: var(--primary-color);">${formatPrice(listing.price)} ETB</span>
                        </div>
                    ...
                    <p style="color: var(--text-secondary); line-height: 1.7; margin-bottom: 2rem; font-size: 1.1rem;">${listing.description}</p>
                ...
                <div class="broker-contact-card">
                    <div style="background: var(--bg-primary); border: 2px solid var(--border-color); border-radius: 16px; padding: 2rem; position: sticky; top: 2rem;">
                        <div style="text-align: center; margin-bottom: 2rem;">
                            <img src="${listing.broker.avatar}" alt="${listing.broker.name}" style="width: 80px; height: 80px; border-radius: 50%; object-fit: cover; margin-bottom: 1rem; border: 3px solid var(--border-color);">
                            <h4 style="margin-bottom: 0.5rem; font-size: 1.3rem; color: var(--text-primary);">${listing.broker.name}</h4>
                            <div style="color: var(--warning-color); margin-bottom: 0.5rem; font-size: 1.1rem;">
                                ${"★".repeat(Math.floor(listing.broker.rating))} ${listing.broker.rating}
                            </div>
                            <div style="color: var(--text-secondary); margin-bottom: 1rem;">${listing.broker.experience} experience</div>
                            ${listing.broker.verified ? '<span style="background: var(--success-color); color: white; font-size: 0.8rem; padding: 0.4rem 0.8rem; border-radius: 20px; font-weight: 600;"><i class="fas fa-check"></i> Verified Broker</span>' : ""}
                        </div>
...
function initializeMovingImages() {
  ...
  console.log("[v0] Moving images initialized with dot navigation and automatic movement")
}
